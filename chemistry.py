...
def _parse_group(text: str, index: int = 0, stop: str | None = None) -> tuple[dict[str, int], int]:
    counts: dict[str, int] = {}
    while index < len(text):
        ...
        if ch == '(' or ch == '[':
            closing = ')' if ch == '(' else ']'
            inner, index = _parse_group(text, index + 1, closing)
...
def parse_formula(formula: str) -> dict[str, int]:
    ...
    counts, end = _parse_group(formula)
    if end != len(formula):
...
def molar_mass(formula: str) -> float:
    ...
    return sum(n * element(sym)["atomic_weight_u"] for sym, n in composition.items())


def mass_to_moles(mass_g: float, formula: str) -> float:
...
def moles_to_mass(moles: float, formula: str) -> float:
...
def balance_reaction(reactants: Sequence[str], products: Sequence[str]) -> tuple[list[int], list[int]]:
    """Balance a chemical equation using exact rational linear algebra.

    Returns integer coefficients in the same order as the supplied species.
    """
    import sympy as sp
    species = list(reactants) + list(products)
    compositions = [parse_formula(f) for f in species]
    elements = sorted({e for c in compositions for e in c})
    matrix = []
    for e in elements:
        row = []
    ...
    left, right = ints[:len(reactants)], ints[len(reactants):]
    if not all(left) or not all(right):
        raise ValueError("Degenerate balance returned zero coefficient")
    return left, right
